...
export const Display = () => {
  ...
  const { status, error, bestSellingProducts } = useSelector(
  ...
  useEffect(() => {
    const fetchDisplayData = async () => {
      if (status == 0) {
        setIsLoading(true);
        dispatch(fetchBestSellingProductThunk());
        if (status == 1) {
          showSuccessNotification("yes its working");
        } else if (success == 2) {
                ...
                <div className="absolute bottom-0 bg-black bg-opacity-80 left-0 h-auto w-full flex flex-col">
                  <div className="h-auto w-auto flex flex-col">
                    ...
                    <span className="h-[30px] w-auto opacity-[80%] text-[0.9rem] text-white ml-3">
                      Master the perfect brew with beans and bite! Learn the are
                      of science of coffee brewing
                    </span>
                  </div>
